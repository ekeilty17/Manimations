...
class SteinerNet2PolarNetImage(ConformalMapScenes):
    ...
    def construct(self):
        ...
        polar_net_text = Text("Polar Net").scale(0.75).next_to(polar_net_image, DOWN)
        polar_net = Group(polar_net_image, polar_net_border, polar_net_text)
        ...
        steiner_net_text = Text("Steiner Net").scale(0.75).next_to(steiner_net_image, DOWN)
        steiner_net = Group(steiner_net_image, steiner_net_border, steiner_net_text)
        ...
        S_text = MathTex("S").next_to(curved_arrow, UP)
        self.add(S_text)
